...
class OISClientRequest:
    ...
    def get_body(self):
        ...
        res["header"]["comCode"] = self.setting["bindComCode"]
        return res
    ...
    def get_candidate_info(self, code):
        ...
        :return:
        """
        self.log.add_log("Request: do get_candidate_info request, code-%s" % code, 1)

        body = self.get_body()
        body["header"]["initRequest"] = True
        command = {
            "commandName": "get_candidate_info",
            "param": {
                "candidateCode": code
            }
        }
        ...
        if res:
            res = r.json()
            if res["header"]["status"] == 0:
                self.log.add_log("Request: get_candidate_info success", 1)
                return res["response"][0]["result"], res["header"]["errorMsg"]
            else:
                self.log.add_log("Request: get_candidate_info failed, err_msg-%s" % res["header"]["errorMsg"], 3)
                return False, res["header"]["errorMsg"]
        else:
            self.log.add_log("Request: get_candidate_info fail. request meet error, code-%s" % r.status_code, 3)
